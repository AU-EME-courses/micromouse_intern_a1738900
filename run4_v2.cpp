...
int stepsrun4 = 0;                     // variable to count number of steps taken in fourth run
...
bool run4 = false; // flag to indicate fourth run with diagonal movement
...
void floodfill(int goalX, int goalY){ // function prototype for floodfill with goal coordinates as parameters{
    ...
    while (!q.empty()) {    // while the queue is not empty, i.e. there are still cells to process
        ...
        // to support 8 directions in run4, otherwise 4 cardinal directions
        int numDirections = run4 ? 8 : 4;  // use 8 directions (including diagonals) in run4, 4 otherwise
        int dx[8] = {0, 1, 0, -1, 1, 1, -1, -1};  // direction deltas for x: N, E, S, W, NE, SE, SW, NW
        int dy[8] = {1, 0, -1, 0, 1, -1, -1, 1};  // direction deltas for y: N, E, S, W, NE, SE, SW, NW

        // local variable direction to explore all directions from the current cell
        for (int direction = 0; direction < numDirections; direction++) {
            int nextX = cellX + dx[direction];  // calculate neighbor cell x coordinate
            int nextY = cellY + dy[direction];  // calculate neighbor cell y coordinate

            // check if path to neighbor is blocked by walls
            bool blocked = false;
            if (direction < 4) {
                // cardinal direction: check wall directly
                blocked = walls[cellY][cellX][direction];
            } else {
                // diagonal direction: check both adjacent walls (NE needs N and E clear, etc.)
                int dir1 = direction - 4;  // first cardinal component (NE->N, SE->E, SW->S, NW->W)
                int dir2 = (direction - 3) % 4;  // second cardinal component (NE->E, SE->S, SW->W, NW->N)
                blocked = walls[cellY][cellX][dir1] || walls[cellY][cellX][dir2];  // blocked if either adjacent wall exists
            }
 
            // if neighbor cell is within maze bounds and there is no wall between current cell and neighbor cell
            if (nextY >= 0 && nextY < maze && nextX >= 0 && nextX < maze && !blocked) {
...
void move1step() {
    ...
    if (!run3 && !run4) {  // if this is the first or second run
        if (visitCount[y][x]==0) {   // if this cell has not been visited before
            recordWalls();
        }
    }
    API::setColor(x, y, !run2 ? 'G' : (!run3 ? 'B' : (!run4 ? 'Y' : 'C'))); // colors the current cell based on the run number (G=green, B=blue, Y=yellow, C=cyan)
    visitCount[y][x]++;  // counter for number of times the cell has been visited
    if ((run3 || run4) && headingtoCentre) { // only record path during third or fourth run to center
    ...
    for (int direction = 0; direction < numDirections; direction++) {  // temporary direction variable 'direction' to explore possible neighbor cells
        int nextX = x + dx[direction];  // calculate neighbor cell x coordinate
        int nextY = y + dy[direction];  // calculate neighbor cell y coordinate

        // check if path to neighbor is blocked by walls
        bool blocked = false;
        if (direction < 4) {
            // cardinal direction: check wall directly
            blocked = walls[y][x][direction];
        } else {
            // diagonal direction: check both adjacent walls (NE needs N and E clear, etc.)
            int dir1 = direction - 4;  // first cardinal component
            int dir2 = (direction - 3) % 4;  // second cardinal component
            blocked = walls[y][x][dir1] || walls[y][x][dir2];  // blocked if either adjacent wall exists
        }
    ...
    // to if run4 and bestdirection is diagonal (4-7), execute as two cardinal moves
    if (run4 && bestdirection >= 4) {
        // map diagonal directions to their two cardinal components
        int moves[4][2] = {{N, E}, {E, S}, {S, W}, {W, N}};  // NE, SE, SW, NW broken into two cardinal moves
        int firstMove = moves[bestdirection - 4][0];  // first cardinal move (e.g., N for NE)
        int secondMove = moves[bestdirection - 4][1];  // second cardinal move (e.g., E for NE)
        
        // execute first cardinal move
        int turns = (firstMove - currentDirection + 4) % 4;  // calculate turns needed
        ...
        currentDirection = static_cast<Direction>(firstMove);  // update direction
        moveForwardStep();  // move forward in first direction
        
        // execute second cardinal move
        turns = (secondMove - currentDirection + 4) % 4;  // calculate turns needed
        if (turns == 3) {  // more efficient to turn left once than right 3 times
            API::turnLeft();
            currentDirection = static_cast<Direction>((currentDirection + 3) % 4);
        } else {
        ...
        currentDirection = static_cast<Direction>(secondMove);  // update direction
        moveForwardStep();  // move forward in second direction
        return;  // diagonal move complete
    }
 
    // turn towards best direction from current direction (cardinal move)
...
int main() {                    
    ...
    while (running) {
        ...
        if (!run2 && !headingtoCentre) {
            ...
            stepsrun2++;
        } else if (run3 && !run4 && !headingtoCentre) {  // to count run3 steps only when not in run4
            stepsrun3++;
        } else if (run4 && !headingtoCentre) {  // to count run4 steps
            stepsrun4++;
        }
        ...
        if (headingtoCentre && x == 0 && y == 0) {
            ...
            } else if (!run4) {  // to start run4 after run3 completes
                headingtoCentre = false;
                run4 = true;   // Start fourth run with diagonal movement
            } else {  // to end after run4 completes
                running = false;   // All runs complete
            }
        }
        if (steps % 10 == 0) {  // print maze every 10 steps
            printmaze();
            std::cerr << "1st run: " << stepsrun1 << ", 2nd run: " << stepsrun2 << ", 3rd run: " << stepsrun3 << ", 4th run: " << stepsrun4 << std::endl;  // display run4 steps
        }
