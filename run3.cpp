...
void move1step() {
    if (!run3) {  // if this is the first run
    ...
    for (int direction = 0; direction < 4; direction++) {  // temporary direction variable 'direction' to explore possible neighbor cells
        ...
        if (nextY >= 0 && nextY < maze && nextX >= 0 && nextX < maze && !walls[y][x][direction]) {
            ...
            if (distance[nextY][nextX] < bestdistance) {
                bestdistance = distance[nextY][nextX];    // update best distance to the new lowest distance found
                bestdirection = direction;    // update best direction to the current direction being checked 'direction'
            }
        }
    }
...
int main() {                    
    ...
    while (running) {          
        ...
        if (!headingtoCentre && x == 8 && y == 8) {  // if the mouse has reached the center
            headingtoCentre = true;    // goal reached, now return to start
        }
